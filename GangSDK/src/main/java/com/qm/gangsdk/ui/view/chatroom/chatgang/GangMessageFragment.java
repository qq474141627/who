...
public class GangMessageFragment extends XLBaseFragment {
    ...
    public void onActivityCreated(@Nullable final Bundle savedInstanceState) {
        ...
        btnAudioRecord.initUIRes(R.drawable.qm_btn_gangchat_press_on,
                R.drawable.qm_btn_gangchat_stop_talk,
                R.color.xlgangchat_button_press_on_color,
                R.color.xlgangchat_button_stop_talk_color);
        //任务完成更新界面
                    ...
                    case XLGangAccidentTaskBean.TYPE_MONSTER_ACCIDENT:
                        for (XLGangAccidentTaskBean accidentBean : accidentTaskList) {
                            if (accidentBean.getType() == XLGangAccidentTaskBean.TYPE_MONSTER_ACCIDENT) {
                                ...
                                setViewTaskMaxHeight(accidentTaskList.size());
        ...
        btnAudioRecord.setAudioFinishRecorderListener(new XLAudioRecordButton.AudioFinishRecorderListener() {
            ...
            public void onStart() {
                ...
                try {
                    if(TimeUtils.isFastDoubleClick(0.5)){
                        return;
                    }
                    mp3Recorder.start();
                } catch (IOException e) {
    ...
    private void sendMessager() {
        ...
        if (sendSuccessFlag) {
            ...
            if(TimeUtils.isFastDoubleClick(10)){
                sendSuccessFlag = false;
            }else {
    ...
    private void setViewTaskMaxHeight(int size){
        if(size >= 3){
            LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, DensityUtil.dip2px(aContext, 150));
            recyclerViewAccidentTask.setLayoutParams(layoutParams);
        }  else {
