...
public class GangConfigureUtils {
    ...
    public static boolean isUsedDynamic(){
        if(!StringUtils.isEmpty(GangSDK.getInstance().groupManager().getGameConfigEntity().getGameconfig().getIs_use_action_module())){
            String is_use_action_module = GangSDK.getInstance().groupManager().getGameConfigEntity().getGameconfig().getIs_use_action_module();
            if(1 == Integer.valueOf(is_use_action_module).intValue() ){
                return true;
            }else {
                return false;
            }
