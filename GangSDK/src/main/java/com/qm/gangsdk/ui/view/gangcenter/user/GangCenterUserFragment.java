...
public class GangCenterUserFragment extends XLBaseFragment implements TakePhoto.TakeResultListener,InvokeListener {
        ...
        if (GangConfigureUtils.isAllowUploadUserHead()) {
            ...
            if (userid == GangSDK.getInstance().userManager().getXlUserBean().getUserid().intValue()) {
                if(!StringUtils.isEmpty(GangSDK.getInstance().groupManager().getGameConfigEntity().getGameprompt().getModify_user_icon())){
                    ...
                    showDialogMemberinfoHint(aContext, spannableStringBuilder, new DialogHintFragment.CallbackOnclick() {
                        @Override
                        public void confirm() {
    ...
    private void isAllowedUseDynaimic(){
        if(GangConfigureUtils.isUsedDynamic()){
            btnGangDynamic.setVisibility(View.VISIBLE);
        }else {
            btnGangDynamic.setVisibility(View.INVISIBLE);
        }
    }
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        btnGangDynamic.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(GangSDK.getInstance().userManager().getXlUserBean() != null) {
                    ...
                    if(!StringUtils.isEmpty(nickname)) {
                        GangModuleManage.toUserDynamicActivity(aContext, userid, nickname);
                    }
                }
            }
        });
     ...
     */
    public static void showDialogMemberinfoHint(Activity activity, CharSequence message, DialogHintFragment.CallbackOnclick callbackOnclick) {
